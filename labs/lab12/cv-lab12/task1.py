...
for fname in fnames:
    ...
    points1 = [keypoints1[m.queryIdx].pt for m in good_matches]  # selects points that were well-matched
    points1 = np.array(points1, dtype=np.float32)
    print(f"Shapes of points1: {points1.shape}")

    points2 = [keypoints2[m.trainIdx].pt for m in good_matches]
    points2 = np.array(points2, dtype=np.float32)
    print(f"Shapes of points2: {points2.shape}")
