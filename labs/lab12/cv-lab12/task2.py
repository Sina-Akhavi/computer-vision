...
for fname in fnames:
    ...
    pts = np.float32([[0, 0],
                      [0, m],
                      [n, 0],
                      [n, m]]).reshape(-1, 1, 2)  # this needs to be changed

    dst = cv2.perspectiveTransform(pts, H).reshape(4, 2).astype(dtype=int)

    J = I2.copy()
    cv2.line(J, (dst[0, 0], dst[0, 1]), (dst[1, 0], dst[1, 1]), (255, 0, 0), 3)
    cv2.line(J, (dst[1, 0], dst[1, 1]), (dst[3, 0], dst[3, 1]), (255, 0, 0), 3)
    cv2.line(J, (dst[3, 0], dst[3, 1]), (dst[2, 0], dst[2, 1]), (255, 0, 0), 3)
    cv2.line(J, (dst[2, 0], dst[2, 1]), (dst[0, 0], dst[0, 1]), (255, 0, 0), 3)

    # I = cv2.drawMatches(I1, keypoints1, J, keypoints2, good_matches, None)

    cv2.imshow('keypoints', J)
